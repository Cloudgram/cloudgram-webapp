...
type SearchItem = {
    type: 'folder' | 'file';
    item: RootFolderType['folders'][0] | RootFolderType['files'][0];
};
...
export const SearchModal = ({ searchQuery, onClose, isOpen }: SearchModalProps) => {
    ...
    useEffect(() => {
        if (data) {
            ...
            setSearchItems([...folders, ...files]);
        } else {
            setSearchItems([]);
        }
    }, [data]);

    const filteredItems = searchItems.filter(({ item, type }) => {
        const searchLower = searchQuery.toLowerCase();
        if (type === 'folder') {
            return (item as RootFolderType['folders'][0]).title.toLowerCase().includes(searchLower);
        }
        return (
            (item as RootFolderType['files'][0]).title.toLowerCase().includes(searchLower) ||
            (item as RootFolderType['files'][0]).extension.toLowerCase().includes(searchLower)
        );
    });
                        ...
                        searchItem.type === 'folder' ? (
                            <li
                                onDoubleClick={e =>
                                    handleFolderDoubleClick(
                                        e,
                                        `/folder/${
                                            (searchItem.item as RootFolderType['folders'][0]).id
                                        }`
                                    )
                                }
                                key={index}
                                ...
                                <div className={styles.search__folder}>
                                    ...
                                    >
                                        <path
                                            ...
                                            fill={
                                                (searchItem.item as RootFolderType['folders'][0])
                                                    .color.back_hex
                                            }
                                        />
                                        <path
                            ...
                            <li
                                style={{
                                    color:
                                        getFileColor(
                                            (searchItem.item as RootFolderType['files'][0])
                                                .extension
                                        )?.color || '',
                                }}
                                onDoubleClick={e =>
                            ...
                            >
                                <div className={styles.search__folder}>
                                    {getFileIcon(
                                        (searchItem.item as RootFolderType['files'][0]).extension
                                    )}
                                    <div className={styles.search__folder__metadata}>
                                        <div className={styles.search__folder__title}>
